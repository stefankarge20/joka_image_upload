...
new Vue({
    ...
    methods: {
        updateProductList: function (categoryName) {
            this.currentArticles = [];
            for (var key in this.articles) {
                var article = this.articles[key];
                if (article.category_name == categoryName || categoryName == "Alle") {
                            ...
                            for(var key in vm.treeData.children){
                                ...
                                if(categoryId == category.id){
                                    for(var key2 in collections){
                                        ...
                                        category.children.push({type: "collection", id: category.id, name: line.name, children:[]});
                                    }
                                }
                            }
                        }).catch(function (error) {
